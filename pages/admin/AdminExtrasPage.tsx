...
import { getExtras, saveExtra, deleteExtra } from '../../services/db';
...
const AdminExtrasPage: React.FC = () => {
  ...
  const confirmDelete = async () => {
    if (deleteConfirmation.id) {
      await deleteExtra(deleteConfirmation.id);
      await refreshExtras();
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingExtra.name && editingExtra.price !== undefined) {
      if (editingExtra.price > 3000) {
        alert("El precio del extra no puede exceder $3,000.");
        return;
      }
      try {
        await saveExtra(editingExtra as Extra);
          ...
          <div>
            ...
            <input
              ...
              onChange={e => {
                ...
                setEditingExtra({ ...editingExtra, price: val === '' ? undefined : Number(val) });
              }}
              required
            />
          </div>
